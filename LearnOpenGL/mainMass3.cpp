...
int main()
{
	...
	Shader shader3("shaders/shader_mass3.vs", "shaders/shader_mass3.fs");
	shader3.use();
	shader3.setBlock("Matrices", 0);
	shader3.setUniformValue("screenTexture", 0);
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
		std::cout << "ERROR::FRAMEBUFFER:: Inrermediate framebuffer is not complete!" << std::endl;
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
